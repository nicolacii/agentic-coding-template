...
**README шаблона vs rules:** если README-инструкции «quick start» противоречат rules в `~/.claude/CLAUDE.md` или `<project>/CLAUDE.md` — **rules авторитетнее**. README — для людей; rules — автоматический контракт для агента.
...
**1c-PG. PRODUCT DECISION GATE — продуктовый контроль (ОБЯЗАТЕЛЬНО до анализа/кода)**

Триггер: задача задевает **user-facing поведение / UX / тексты ИЛИ scope (что входит в MVP)**.
Чисто технические задачи (рефакторинг, багфикс без UX-изменения) — gate пропускается.

Классификация решений ПЕРЕД работой:

| Тип решения | Правило |
|-------------|---------|
| **Продуктовые** — scope, in/out MVP, поведение для пользователя, UX-флоу, тексты/копирайт, видимые пользователю данные/названия, trade-offs которые важны пользователю | **ВСЕГДА выносить пользователю на явный выбор** (вопросы с вариантами). НИКОГДА не решать молча. |
| **Технические** — выбор библиотеки, раскладка файлов, внутренние имена, алгоритм, структура тестов | Можно выбрать дефолт, но **обязательно озвучить** в плане/ответе, чтобы можно было наложить вето. |

Процесс gate:
1. **Brainstorm интента** — уточняющие вопросы ПО ОДНОМУ за раз, multiple-choice предпочтительно. Цель: понять цель/ограничения/критерии успеха.
2. **Список продуктовых решений** — каждое с вариантами и моей рекомендацией.
3. **HARD-GATE:** никакого анализа архитектуры / кода / скаффолда, пока пользователь не утвердил продуктовые решения.
4. **Decisions Log** — записать в task-файл: какое решение, кто решил (пользователь / AI-default).

🚩 Red flags тихих допущений (СТОП и спроси, а не решай сам):
- «Я предположил…» / «Логично предположить…» / «Для удобства я…» / «Очевидно, пользователь хочет…»
- Выбрал ОДИН вариант там, где есть 2+ разумных продуктовых варианта, не спросив.

> Размещение в pipeline: это **Stage 1 (BRAINSTORM + TASK)** в WORKFLOW.md — ДО Stage 2 ANALYSIS. Анализ = код/архитектура, пользователь там не участвует; он получает уже детализированную постановку.
...
**Continuous execution (внутри утверждённого плана)** — источник: Superpowers `subagent-driven-development`:
- Если план уже утверждён (продуктовые решения через Product Gate приняты) — исполнять задачи плана **подряд, без чек-инов** «продолжать?» между задачами.
- DONE-блок — только на **границе ответа пользователю**, не после каждой внутренней задачи плана.
- Стоп между задачами ТОЛЬКО при: BLOCKED (не можешь решить), genuine ambiguity, план завершён.
- ⚠️ Это НЕ отменяет Product Gate (ШАГ 1c-PG): продуктовые решения всё равно до плана и до кода.
