...
### Step 1.5: PRODUCT DECISION GATE (если задача user-facing / задевает scope)

> Соответствует Stage 1 (BRAINSTORM + TASK) в WORKFLOW.md. Цель — продуктовый контроль: НЕ решать продуктовые вопросы за пользователя. См. global CLAUDE.md → «1c-PG».

Триггер: user-facing поведение / UX / тексты ИЛИ scope. Технические задачи (рефакторинг, багфикс без UX) — пропустить.

```markdown
## PRODUCT GATE
**Brainstorm:** уточняющие вопросы пользователю ПО ОДНОМУ (multiple-choice), пока не ясны цель/scope/UX.
**Product decisions (на апрув пользователю):**
| # | Решение | Варианты | Моя рекомендация |
|---|---------|----------|------------------|
| 1 | [scope / UX / текст / поведение] | A / B / C | [рек.] |
**Technical defaults (озвучить, не блокировать):** [библиотека, раскладка, имена…]
**Decisions Log (кто решил):** [#1 — пользователь / AI-default]
```

🚦 **HARD-GATE:** Step 2 PLAN, ANALYSIS, любой код — ТОЛЬКО после явного апрува продуктовых решений пользователем.
🚩 Тихие допущения («я предположил», «для удобства», «логично») → СТОП, вынести вопрос.
...
## TDD Anti-Rationalization (броня)

> Усиление TDD против отговорок под давлением. Источник: Superpowers `test-driven-development`.
> **Нарушение буквы правил TDD = нарушение духа правил TDD.**

### The Iron Law
```
NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST
```
Написал код до теста? **Удали. Начни заново.** Без исключений:
- Не держи «как reference». Не «адаптируй» пока пишешь тесты. Не смотри на него. Delete means delete.
- Реализуй заново из тестов.

### Watch it fail (Verify RED — ОБЯЗАТЕЛЬНО, никогда не скипать)
Если ты не **видел**, как тест падает, ты не знаешь, тот ли он тест. Тест прошёл сразу → он тестирует
существующее поведение, а не новое → **чини тест**. Тест выдал error (не fail) → чини error, гоняй пока
не упадёт по правильной причине (фича отсутствует, а не опечатка).

### Исключения (только с разрешения пользователя)
Throwaway-прототип · сгенерированный код · конфиг-файлы. Думаешь «скипну TDD один раз»? Это рационализация.

### Rationalization table
| Отмазка | Реальность |
|--------|-----------|
| «Слишком просто чтобы тестировать» | Простой код ломается. Тест — 30 секунд. |
| «Напишу тесты после» | Тесты, прошедшие сразу, ничего не доказывают. |
| «Тесты-после дают ту же цель» | После = «что это делает?». До = «что это ДОЛЖНО делать?». |
| «Уже проверил вручную» | Ad-hoc ≠ систематически. Нет записи, нельзя перезапустить. |
| «Удалять X часов работы — расточительство» | Sunk cost. Непроверяемый код = техдолг. |
| «Оставлю как reference, тесты потом» | Ты его адаптируешь = тесты-после. Delete means delete. |
| «Тест писать сложно» | Слушай тест: сложно тестировать = сложно использовать. Упрости интерфейс. |
| «TDD догматичен, я прагматичен» | TDD прагматичнее отладки в проде. |

### Red Flags — STOP и начни заново
Код до теста · тест после реализации · тест проходит сразу · не могу объяснить почему тест упал ·
«тесты добавлю позже» · «уже проверил вручную» · «это про дух, а не ритуал» · «оставлю как reference» ·
«уже потратил X часов» · «это другой случай, потому что…». **Всё это значит: удалить код, начать с TDD.**

### Когда застрял
| Проблема | Решение |
|----------|---------|
| Не знаю как тестировать | Напиши желаемый API. Сначала assertion. Спроси пользователя. |
| Тест слишком сложный | Дизайн слишком сложный. Упрости интерфейс. |
| Нужно мокать всё | Код слишком связан. Dependency injection. |
